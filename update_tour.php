...
if(isset($_POST["btnsubmit"])){
  ...
  //  $loi_str = "";
    // Lưu xuống CSDL
    $result = $newTour -> update($maTour, $maPhuongTien, $tenTour, $noiKhoiHanh, $ngayKhoiHanh, $thoiGian, $giaTour, $soLuongVeHienCo, $moTa, $hinh);
    if(!$result){
        //Truy vấn lỗi
      // header("Location: add_product.php?status=failure");
      // foreach($loi as $item) $loi_tr = $loi_tr.$item."<br/>";
      ?>
      <script>alert('Sửa tour thất bại!');</script>
      <?php

    }else{
        //header("Location: add_product.php?status=inserted");
        ?>  
        <script>alert('Sửa tour thành công!');</script>
        <?php
