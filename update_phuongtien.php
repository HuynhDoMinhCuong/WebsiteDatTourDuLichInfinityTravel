...
<head>
   ...
   <!-- <link href="site.css" rel="stylesheet" /> -->
    <title>Update phương tiện</title>
...
if(isset($_POST["btnsubmit-capnhatphuongtien"])){
    //Lấy giá trị từ form
  ...
  //  $loi_str = "";
    // Lưu xuống CSDL
    $result = $newPhuongTien -> update($maPhuongTien, $tenPhuongTien);
    if(!$result){
        //Truy vấn lỗi
      // header("Location: add_product.php?status=failure");
      // foreach($loi as $item) $loi_tr = $loi_tr.$item."<br/>";
      ?>
      <script>alert('Sửa phương tiện thất bại!');</script>
      <?php

    }else{
        //header("Location: add_product.php?status=inserted");
        ?>  
        <script>alert('Sửa phương tiện thành công!');</script>
        <?php
  ...
  <form method="post">
    ...
    <div class="row">
        ...
        <div class="row">
            ...
            <div class="submit">
                <button type="submit" class="btn btn-primary" name="btnsubmit-capnhatphuongtien">Cập nhật</button>
            </div>
        </div>
    </form>
    </div>
